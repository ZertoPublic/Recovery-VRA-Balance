...
$maxVMs = "16" #Max Number of VMs per Recovery VRA (Host)
...
function getReport ($TargetHostName){
    ...
    $i = 0
    # QueryResourceReport
    $url = "https://" + $ZertoServer + ":"+$ZertoPort + "/ZvmService/ResourcesReport/getSamples?fromTimeString=" + $StartDateTime + "&toTimeString=" + $EndDateTime + "&startIndex=0&count=500"
    $response = Invoke-RestMethod -Uri $url -TimeoutSec 100 -Headers $zertoSessionHeader -ContentType "application/json"
	    ForEach ($vm in $response) {
            if ($vm.TargetHost -eq $TargetHostName){
            $i++
        }
